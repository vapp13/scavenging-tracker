...
export class ChatboxWatcher {
    constructor(tracker) {
        ...
        this._prevSnapshot        = [];
        this._processedTimestamps = new Set(); // timestamp-based dedup
    }
    ...
    _tick() {
        try {
            ...
            for (const text of newLines) {
                // Extra dedup layer: if the chat has local timestamps enabled,
                // use [HH:MM:SS] + content as a unique key so the same message
                // is never counted twice even if the snapshot diff lets it through.
                const tsKey = this._makeTimestampKey(text);
                if (tsKey) {
                    if (this._processedTimestamps.has(tsKey)) {
                        console.log('[ScavTracker] Skipping duplicate by timestamp:', tsKey);
                        continue;
                    }
                    this._processedTimestamps.add(tsKey);
                }
    ...
    /**
     * Build a deduplication key from the chat timestamp and content.
     * Returns null if no timestamp is present (timestamps may be disabled).
     * Format: "[HH:MM:SS]|content without timestamp"
     * @param {string} line
     * @returns {string|null}
     */
    _makeTimestampKey(line) {
        const m = line.match(/\[(\d{2}:\d{2}:\d{2})\]/);
        if (!m) return null;
        const content = line.replace(/\[\d{2}:\d{2}:\d{2}\]\s*/, '').trim();
        return `${m[1]}|${content}`;
    }
